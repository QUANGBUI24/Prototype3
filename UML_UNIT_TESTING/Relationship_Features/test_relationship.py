...
class TestUMLRelationship(unittest.TestCase):
    def setUp(self):
        ...
        self.patcher_load = patch(
            "UML_UTILITY.SAVE_LOAD.save_load.load_data_from_json",
            return_value=(self.sample_data, UMLClass.data_list[1]),
        )
    ...
    def test_check_format_invalid(self):
        ...
        with io.StringIO() as fake_out:
            ...
            for name in invalid_names:
                ...
                self.assertEqual(
                    result,
                    "Invalid format. Only lowercase alphabet characters are allowed.",
                )
    ...
    def test_add_relationship_valid(self):
        ...
        self.assertIn(
            {"source": "person", "dest": "cat", "relation": "owns"},
    ...
    def test_add_relationship_existing(self):
        ...
        UMLRelationship.relationship_list.append(
            {"source": "person", "dest": "cat", "relation": "owns"}
        )
    ...
    def test_add_relationship_invalid_source_class(self):
        ...
        with patch("builtins.input", side_effect=["Yes"]):
            with io.StringIO() as fake_out:
                ...
                self.assertNotIn(
                    {"source": "invalidsource", "dest": "cat", "relation": "owns"},
                    UMLRelationship.relationship_list,
                )
    ...
    def test_add_relationship_invalid_dest_class(self):
        ...
        with patch("builtins.input", side_effect=["Yes"]):
            with io.StringIO() as fake_out:
                ...
                self.assertNotIn(
                    {"source": "person", "dest": "invaliddest", "relation": "owns"},
                    UMLRelationship.relationship_list,
                )
    ...
    def test_add_relationship_empty_class_name(self):
        with io.StringIO() as fake_out:
            ...
            output = fake_out.getvalue()
            self.assertIn(
                "Invalid length. Must be between 2 and 50 characters.", output
            )
