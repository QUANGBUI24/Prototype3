...
def add_param(class_name: str, method_name: str, param_name: str):
    ...
    # Check if method  already exists
    # if not, called function will print error, current function ends
    is_method_exist = check_method_name(method_list, method_name, should_exist=True)
    if not is_method_exist:
        return
    # Check if parameter already exists
    # if not, called function will print error, current function ends
    is_param_exist = check_param_name(param_name, should_exist=False)
...
def check_param_name(
    ...
    # If the method name should exist but not exist
    if should_exist and not is_method_exist:
...
def get_param_list(class_name: str, method_name: str) -> list[str]:
    method_list = get_method_list(class_name)
    for dictionary in method_list:
        if dictionary["method_name"] == method_name:
